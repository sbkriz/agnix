...
### Changed
- **API**: Removed `#[non_exhaustive]` from `ValidationResult` struct - all fields were already public, so the attribute only prevented struct literal construction which no consumer needs (closes #487)
